...
const newProductController = async (req, res, next) => {
    try {
        ...
        const checkImage = await validateSchema(newProductSchema, {
            ...req.body,
            ...req.files,
        });
        console.log('aaaa', checkImage);
        console.log('eeee', checkImage?.status);
        if (checkImage?.status === 'error') {
            console.log('iiii', 'Entra por aquí');
            res.send({
                status: 'Error',
                message: checkImage.message,
            });
            return;
        }
        console.log('oooo', 'No debería entrar por aquí');
