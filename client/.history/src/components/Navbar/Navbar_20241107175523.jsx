...
const Navbar = () => {
    const { authUser, authLogout, loading } = useAuth();
    ...
    return (
        <nav className="navbar shadow-xl shadow-black bg-slate-900 bg-transparent backdrop-blur-md ">
            ...
            >
                <button
                    className={`upload-product md:w-[230px] md:h-[60px] p-2 text-xs md:text-lg cursor-pointer shadow-xl shadow-black border border-slate-500 ${
                        !loading ? 'flex' : 'hidden'
                    }`}
                    disabled={!authUser}
                >
